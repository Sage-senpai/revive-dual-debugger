...
const dualTracePanel_1 = __webpack_require__(/*! ./webview/dualTracePanel */ "./src/webview/dualTracePanel.ts");
...
function activate(context) {
    weightMeter = new weightMeter_1.WeightMeter();
    // ── Status Bar ────────────────────────────────────────────────────────────
    ...
    context.subscriptions.push(vscode.debug.registerDebugAdapterDescriptorFactory('revive', factory));
    // ── Debug Session Lifecycle ───────────────────────────────────────────────
    context.subscriptions.push(vscode.debug.onDidStartDebugSession(session => {
        if (session.type === 'revive') {
            updateStatusBar('running');
            dualTracePanel_1.DualTracePanel.createOrShow(context.extensionUri, weightMeter);
        }
        ...
        if (event.session.type !== 'revive')
            return;
        handleCustomEvent(event, context);
    }));
    // ── Commands ──────────────────────────────────────────────────────────────
    context.subscriptions.push(vscode.commands.registerCommand('revive.openDualTrace', () => {
        dualTracePanel_1.DualTracePanel.createOrShow(context.extensionUri, weightMeter);
...
// ─── Debug Adapter Factory ────────────────────────────────────────────────────
class ReviveDebugAdapterFactory {
...
async function startNodeCommand(context) {
    ...
    orchestrator = new nodeOrchestrator_1.NodeOrchestrator(orchConfig);
    ...
    orchestrator.on('ready', () => {
        updateStatusBar('running');
        vscode.window.showInformationMessage('Revive dev node is running', 'Open Trace View').then(choice => {
...
async function stopNodeCommand() {
    if (!orchestrator) {
...
async function switchBackendCommand(backend) {
    const session = vscode.debug.activeDebugSession;
    if (!session || session.type !== 'revive') {
        vscode.window.showWarningMessage('No active Revive debug session. Start debugging first.');
        return;
    }
    await session.customRequest('switchBackend', { backend });
    const panel = dualTracePanel_1.DualTracePanel.getInstance();
    if (panel) {
        panel.clear();
    }
...
async function deployAndDebugCommand(context) {
    ...
    const config = vscode.workspace.getConfiguration('revive');
    const defaultBackend = config.get('defaultBackend', 'BOTH');
    const launchConfig = {
        type: 'revive',
        request: 'launch',
        name: 'Deploy & Debug',
...
async function compileContractCommand() {
    const editor = vscode.window.activeTextEditor;
    if (!editor || !editor.document.fileName.endsWith('.sol')) {
    ...
    if (!choice)
        return;
    await vscode.window.withProgress({ location: vscode.ProgressLocation.Notification, title: 'Compiling...', cancellable: false }, async () => {
        try {
            const { ContractDeployer } = await Promise.resolve().then(() => __importStar(__webpack_require__(/*! ./contractDeployer */ "./src/contractDeployer.ts")));
            ...
            vscode.window.showInformationMessage(`Compiled ${result.contractName} successfully` +
                (result.evm ? ' [EVM]' : '') +
                (result.pvm ? ' [PVM]' : ''));
        }
        catch (err) {
...
function handleCustomEvent(event, _context) {
    const panel = dualTracePanel_1.DualTracePanel.getInstance();
    if (!panel)
        return;
...
// ─── Status Bar ───────────────────────────────────────────────────────────────
function updateStatusBar(state, backend) {
    switch (state) {
        case 'idle':
...
Object.defineProperty(exports, "__esModule", ({ value: true }));
exports.DualTracePanel = void 0;
exports.getDualTraceHtml = getDualTraceHtml;
const vscode = __importStar(__webpack_require__(/*! vscode */ "vscode"));
const path = __importStar(__webpack_require__(/*! path */ "path"));
const fs = __importStar(__webpack_require__(/*! fs */ "fs"));
// ─── DualTracePanel ───────────────────────────────────────────────────────────
class DualTracePanel {
    constructor(panel, weightMeter, extensionUri) {
        // Trace state
        this.revmTrace = [];
        this.pvmTrace = [];
        this.panel = panel;
        ...
        panel.webview.onDidReceiveMessage((message) => {
            switch (message.command) {
                case 'jumpToSource':
                    this.handleJumpToSource(message.data);
                    break;
                case 'copyTrace':
                    this.handleCopyTrace(message.data);
                    break;
            }
        });
    }
    // ─── Static Factory ────────────────────────────────────────────────────────
    static createOrShow(extensionUri, weightMeter) {
        ...
        const panel = vscode.window.createWebviewPanel('reviveDualTrace', 'Revive Dual-VM Trace', vscode.ViewColumn.Beside, {
            enableScripts: true,
    ...
    updateRevmTrace(metrics) {
        if (!metrics.opcodeTrace)
            return;
    ...
    updatePvmTrace(metrics) {
        if (!metrics.syscallTrace)
            return;
    ...
    getHtml() {
        ...
        const htmlPath = path.join(this.extensionUri.fsPath, 'src', 'webview', 'dualTrace.html');
...
exports.DualTracePanel = DualTracePanel;
// ─── Inline HTML (fallback) ───────────────────────────────────────────────────
function getDualTraceHtml() {
    return `<!DOCTYPE html>
<html lang="en">
<head>
  <meta charset="UTF-8">
